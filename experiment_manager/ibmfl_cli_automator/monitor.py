...
if __name__ == "__main__":
    ...
    if args.action == "daemonize":
        ...
        if "timestamp" in config:
            ...
            cmds = [
                list_cmd.format(config["local_staging_dir"], u, m, u, run_id, run_id, u, run_id, run_id)
                for m, u in zip(machines, usernames)
            ]
        else:
            list_cmd = (
                'ssh -o "ControlMaster=no" -o "ControlPath={}/.ssh/master-%r@%h:%p" {}@{} '
        ...
        if "timestamp" in config:
            ...
            cmds = [
                kill_cmd.format(config["local_staging_dir"], u, m, u, run_id, run_id, u, run_id, run_id)
                for m, u in zip(machines, usernames)
            ]
        else:
            kill_cmd = (
