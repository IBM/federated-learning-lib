...
from ibmfl.util.datasets import load_adult, load_nursery
...
def get_model_config(folder_configs, dataset, is_agg=False, party_id=0, model=None):
    ...
    for feature in range(loaded_data.shape[1]):
        if loaded_data.columns[feature] != "class":
            new_feature = loaded_data[loaded_data.columns[feature]].cat.categories
    ...
    spec = {"model_name": "decision-tree", "model_definition": os.path.join(folder_configs, "dt_model_spec.json")}

    model = {"name": "DTFLModel", "path": "ibmfl.model.dt_fl_model", "spec": spec}
