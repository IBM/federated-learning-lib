...
def setup_parser():
    ...
    p.add_argument(
        "--model",
        "-m",
        help=MODEL_CONFIG_DESC,
        choices=[os.path.basename(d) for d in FL_MODELS],
        required=False,
        default=None,
    )
    p.add_argument(
        "--fusion", "-f", help=FUSION_CONFIG_DESC, required=True, choices=[os.path.basename(d) for d in FL_EXAMPLES]
    )
    ...
    p.add_argument(
        "--connection",
        "-c",
        choices=[os.path.basename(d) for d in FL_CONN_TYPES],
        help=CONNECTION_TYPE_DESC,
        required=False,
        default="flask",
    )
...
def generate_connection_config(conn_type, party_id=0, is_party=False, task_name=None):
    ...
    if conn_type == "pubsub":
        connection = {"name": "RabbitMQConnection", "path": "ibmfl.connection.rabbitmq_connection", "sync": True}
        if is_party:
            connection["info"] = {"credentials": f"party{party_id}.json", "role": "party", "task_name": task_name}
        else:
            connection["info"] = {"credentials": "aggregator.json", "role": "aggregator", "task_name": task_name}
...
def generate_agg_config(module, model, num_parties, conn_type, dataset, folder_data, folder_configs, task_name=None):
    ...
    print(
        "Finished generating config file for aggregator. Files can be found in: ",
        os.path.abspath(os.path.join(folder_configs, "config_agg.yml")),
    )


def generate_party_config(module, model, num_parties, conn_type, dataset, folder_data, folder_configs, task_name=None):
...
def generate_crypto_keys(module, config_path):
...
if __name__ == "__main__":
    ...
    if create_new:
        folder_configs = os.path.join(folder_configs, exp_name if exp_name else str(int(time.time())))
    else:
    ...
    generate_crypto_keys(config_fusion, config_path)

    generate_agg_config(
        config_fusion, model, num_parties, conn_type, dataset, party_data_path, folder_configs, task_name
    )
    generate_party_config(
        config_fusion, model, num_parties, conn_type, dataset, party_data_path, folder_configs, task_name
    )
